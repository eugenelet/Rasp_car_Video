...
/*	for (multiset<OMG>::reverse_iterator p = forSort.rbegin(); p != forSort.rend(); ++p){
	...
	for (multiset<OMG>::reverse_iterator p = forSort.rbegin(); p != forSort.rend(); ++p){
		//Mat tempImage = imageColor;
		if(p->Rvalue!=INT_MAX /*&& p->Rvalue!=4.0*/ && p->Rvalue<12){
			// circle(imageColor, Point(p->col, p->row), 3, Scalar(255, 255, 0), 1);
			//imshow("??", imageColor);
			//waitKey(1);
			// printf("%f %d\n", p->Rvalue, index);
...
void computeDxxDxyDyy(Mat& src, Mat& Dxx, Mat& Dxy, Mat& Dyy)
{
	...
	for (int row = 0; row < src.rows; ++row)
		for (int col = 1; col < src.cols - 1; ++col)
			Dxx.at<float>(row, col) = src.at<uchar>(row, col - 1) + src.at<uchar>(row, col + 1) - 2 * src.at<uchar>(row, col);
	...
	for (int row = 1; row < src.rows - 1; ++row)
		for (int col = 0; col < src.cols; ++col)
			Dyy.at<float>(row, col) = src.at<uchar>(row - 1, col) + src.at<uchar>(row + 1, col) - 2 * src.at<uchar>(row, col);
